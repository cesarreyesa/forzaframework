    ...
    ${class.module.jsNamespace}.${class.name} = function(){
        ...
        var edit = function(){
            if(grid.getSelectionModel().getSelected()()){
                link('<c:url value="${class.module.webFolder}${class.name.toLowerCase()}Form.html"/>?id=' + grid.getSelectionModel().getSelected().id);
            }
        }
        return {
            init : function(){
...
//                var keyMap = new YAHOO.ext.KeyMap(formPanel, [{ key: [10,13], fn: this.saveAdd, scope: formPanel },
//                                                          { key: 27, fn: formPanel.hide, scope: formPanel }], "keyup");
            },
            setGrid : function(_grid) { grid = _grid; },
            showSearch: function(){
                getEl(searchPanel.id).toggle();
            },
            add : function() {
                updatePanel(formPanel.id, '<c:url value="/${class.module.webFolder}${class.name.toLowerCase()}Form.html"/>?${parent.name.toLowerCase()}=$${blank}{param.${parent.name.toLowerCase()}}', function(){
#foreach( $att in ${class.attributes})
#if($velocityCount == 1)
                    getEl('${class.name.toLowerCase()}.${att.name.toLowerCase()}').focus();
#end
#end
                });
            },
            saveAdd : function() {
                doAjaxSubmit('validate', $('${class.name.toLowerCase()}Form'), $('${class.name.toLowerCase()}-save-btn'), null, function(){
                    getEl('${class.name.toLowerCase()}Edit').hide();
                    formPanel.hide();
                    grid.getDataSource().load();
                });
            },
...
#foreach( $att in ${class.attributes})
    ...
    <n:grid id="${class.name.toLowerCase()}" jsNamespace="${class.module.jsNamespace}.${class.name}" url="${ctx}/${class.module.webFolder}${class.name.toPluralFirstToLower()}List.html?format=Xml&${parent.name.toLowerCase()}=$${blank}{param.${parent.name.toLowerCase()}}">
#foreach( $att in ${class.attributes})
