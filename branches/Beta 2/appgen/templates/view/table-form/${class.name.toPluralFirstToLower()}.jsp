...
//                                                          { key: 27, fn: formPanel.hide, scope: formPanel }], "keyup");
            ...
            add : function() {
                updatePanel(formPanel.id, '<c:url value="/${class.module.webFolder}${class.name.toLowerCase()}Form.html"/>', function(){
...
#foreach( $att in ${class.attributes})
    ...
    <n:grid id="${class.name.toLowerCase()}" jsNamespace="${class.module.jsNamespace}.${class.name}" url="${ctx}/${class.module.webFolder}${class.name.toPluralFirstToLower()}List.html?format=Xml">
#foreach( $att in ${class.attributes})
